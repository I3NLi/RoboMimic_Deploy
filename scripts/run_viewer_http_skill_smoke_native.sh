...
for _ in $(seq 1 80); do
    if curl -sf "${status_url}" -o "${status_body}" &&
       jq -e '.mode == "SKILL" and .external_policy == "TrackMimic" and .paused == false and .http_control_commands >= 1 and .policy_steps > 0 and .sim_steps > 0' "${status_body}" >/dev/null; then
        status_ready=1
        break
    fi
    sleep 0.1
...
if ! jq -e '.mode == "SKILL" and .external_policy == "TrackMimic" and .paused == false and .policy_steps > 0 and .sim_steps > 0 and .http_control_commands >= 1' "${summary_json}" >/dev/null; then
