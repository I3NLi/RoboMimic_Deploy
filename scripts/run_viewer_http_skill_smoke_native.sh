...
if [[ -z "${summary_json}" ]]; then
    summary_json="$(mktemp /tmp/magicbot_viewer_http_skill_XXXXXX.json)"
fi
...
status="$(curl -sS -o "${status_body}" -w '%{http_code}' -X POST "${control_url}?mode=track_mimic")"
if [[ "${status}" != "200" ]]; then
    echo "[Smoke][ERROR] track_mimic control returned HTTP ${status}" >&2
...
for _ in $(seq 1 80); do
    if curl -sf "${status_url}" -o "${status_body}" &&
       jq -e '.mode == "SKILL" and .paused == false and .http_control_commands >= 1 and .policy_steps > 0 and .sim_steps > 0' "${status_body}" >/dev/null; then
        status_ready=1
        break
    fi
    sleep 0.1
...
if [[ "${status_ready}" -ne 1 ]]; then
    echo "[Smoke][ERROR] viewer /status did not report active SKILL" >&2
    cat "${status_body}" >&2 || true
...
if ! jq -e '.mode == "SKILL" and .paused == false and .policy_steps > 0 and .sim_steps > 0 and .http_control_commands >= 1' "${summary_json}" >/dev/null; then
...
jq '{mode, paused, sim_steps, policy_steps, http_control_commands}' "${summary_json}"
