#!/usr/bin/env bash
# scripts/compare_python_cpp.sh
# Python MuJoCo DDS bridge vs C++ robot_controller_onnx shadow compare launcher.
#
# Modes
# -----
...
#   bash scripts/compare_python_cpp.sh [BeyondMimic.yaml] [net_iface]
#
# Verify usage (recommended):
#   bash scripts/compare_python_cpp.sh --verify --yaml policies/beyond_mimic/config/BeyondMimic.yaml --net lo
...
usage() {
    cat <<'EOF'
Usage:
  bash scripts/compare_python_cpp.sh [YAML] [NET] [options]
...
MODE="interactive"
YAML="${REPO_ROOT}/policies/beyond_mimic/config/BeyondMimic.yaml"
TRACK_YAML=""
SHADOW_STATE="beyond"
NET="wlp3s0"
JOINTS="24"
CPP_BIN="${REPO_ROOT}/controller_cpp/build_z1/robot_controller_onnx"
if [ ! -f "$CPP_BIN" ]; then
    CPP_BIN="${REPO_ROOT}/controller_cpp/build/robot_controller_onnx"
fi
PY_SCRIPT="${REPO_ROOT}/python_reference/simulation/mujoco_dds_compare.py"
CONDA_ENV="robomimic"
SYNC_LOWSTATE=0
LAG_SEARCH=-1
CMD_WAIT_MS="5.0"
...
if [ "$SHADOW_STATE" = "track" ]; then
    if [ -z "$TRACK_YAML" ]; then
        TRACK_YAML="${REPO_ROOT}/policies/track_mimic/config/BeyondMimic.yaml"
    fi
...
if [ ! -f "$CPP_BIN" ]; then
    ...
    echo "        Run: cmake -S controller_cpp -B controller_cpp/build_z1 && cmake --build controller_cpp/build_z1"
    exit 1
fi
if [ ! -f "$PY_SCRIPT" ]; then
