...
behavior, explicit held-target forwarding, and a STAND target-rate clamp check
inside `ControllerCore`. It registers fake DANCE/SKILL external policies to
verify keyed policy selection, reset/step calls, entry command zeroing, and
completion back to a damping mode.
