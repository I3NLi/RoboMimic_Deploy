...
through `ControllerRuntime` and `MagicbotRealAdapter`; debug/passive damping and
the final damping cleanup also write through the same runtime/adapter boundary.
The existing staged safety flow is preserved: dry-run, connect-check,
read-state, debug/passive damping, stand interpolation, and PD stand-only remain
outside high-risk LOCO execution.
