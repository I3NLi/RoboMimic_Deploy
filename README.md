...
```text
/health
/status         JSON: viewer, adapter, command, mode, target_mode, safety, and camera telemetry
/frame.jpg
/frame.png
/stream.mjpg
...
`axis1` is `vx`, `axis2` is `vy`, and `axis0` is `wz`. Physical `A` enters
`LOCO` (button 0), `X` enters `STAND` (button 3), `Y` requests
