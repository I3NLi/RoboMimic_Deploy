 ...
 * deploy_mujoco/deploy_mujoco_dds.py:
 ...
 * real robot. Run it with deploy_real_onnx --shadow to exercise the C++ policy
...
struct Args {
    std::string net = "lo";
    std::string mujoco_yaml = "deploy_mujoco/config/mujoco.yaml";
    std::string xml = "";
    std::string yaml = "policy/beyond_mimic/config/BeyondMimic.yaml";
...
fs::path repo_root()
{
    ...
    if (fs::exists(cwd / "deploy_mujoco") && fs::exists(cwd / "magicbot_z1_description")) {
        return cwd;
    }
    fs::path p = fs::absolute(fs::path(__FILE__)).parent_path().parent_path();
    return p;
}
...
int main(int argc, char** argv)
{
    ...
    if (args.xml.empty()) {
        args.xml = "magicbot_z1_description/scene.xml";
    }
